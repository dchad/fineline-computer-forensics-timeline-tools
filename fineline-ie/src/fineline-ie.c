...
int main(int argc, char *argv[])
{
   ...
   if (mode > 0)
   {
      if (mode & FL_CACHE_IN) /* Parse WebCacheV01.dat */
      {
         parse_ie_cache_file(url_cache_in_file, fl_out_file, mode, gui_ip_address, filter_file);
      }
      else
      {
...
int parse_command_line_args(int argc, char *argv[], char *fl_filename, char *in_file, char *gui_ip_address, char *filter_file)
{
   ...
   {
      strncpy(in_file, IE_CACHE_FILE, strlen(IE_CACHE_FILE));
      /* IE1-9 = index.dat, IE10+ = WebCacheV01.dat */
      retval = retval | FL_CACHE_IN;      
   }
